    ...
    def decoding(self, encoder_hs):
        ...
        with tf.variable_scope(name_or_scope='decoder_convlstm',reuse=tf.AUTO_REUSE):
            ...
            for i in range(self.predict_time):
                ...
                max_pool = tf.nn.avg_pool(tf.squeeze(h_state,axis=1), ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding='SAME')
                cnn_shape = max_pool.get_shape().as_list()
                nodes = cnn_shape[1] * cnn_shape[2] * cnn_shape[3]
                results = tf.layers.dense(inputs=tf.reshape(max_pool, [cnn_shape[0], nodes]), units=256, name='layer_1', reuse=tf.AUTO_REUSE,activation=tf.nn.relu)
